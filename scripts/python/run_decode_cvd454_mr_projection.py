#!/usr/bin/env python3
"""D035 — CVD454 AF MR + candidate-only HF/stroke projection.

Runs after `run_decode_cvd454_extraction.py` completes. Stages:

  1. Load `data/processed/instruments/decode_cvd454_instruments.parquet`.
  2. Per aptamer, harmonize against AF.parquet and run MR (Wald or IVW +
     weighted-median sensitivity if >=3 instruments).
  3. BH-FDR within the CVD454 aptamer universe (NOT proteome-wide).
  4. Build candidate shortlist:
       AF_FDR_candidate     : FDR_AF < 0.05
       AF_nominal_candidate : p_AF < 0.05 and FDR_AF >= 0.05
       AF_no_signal         : p_AF >= 0.05
       Plus pre-specified sanity targets always carried into projection.
  5. For candidate aptamers, project onto HF (3 primary + HFpEF exploratory)
     and stroke (4 primary). Compute theta = d_T * beta_outcome and OR.
  6. Emit reports + candidate summary markdown.

Coloc remains LOCKED. Final classification (GREEN/RED/AMBER/BLUE/PURPLE)
LOCKED. Protocol explicit approval required to unlock 3D.
"""
...
INSTR_PARQUET = ROOT / "data/processed/instruments/decode_cvd454_instruments.parquet"
...
OUT_AF_HARM = ROOT / "results/qc/decode_cvd454_to_AF_harmonization_qc.tsv"
OUT_AF_MR = ROOT / "results/mr/decode_cvd454_pqtl_to_AF.tsv"
OUT_CANDIDATES = ROOT / "results/mr/decode_cvd454_AF_candidate_shortlist.tsv"
OUT_PROJ = ROOT / "results/mr/decode_cvd454_candidate_hf_stroke_projection.tsv"
OUT_DIRECTION = ROOT / "results/mr/decode_cvd454_candidate_direction_table.tsv"
OUT_SUMMARY = ROOT / "results/mr/decode_cvd454_candidate_summary.md"

# Sanity targets always carried into projection regardless of AF MR signal.
SANITY_TARGETS = {"IL6R", "NPPA", "NPPB", "PCSK9", "F11", "F2", "F9", "ADAMTS13"}
...
def _log(msg: str) -> None:
    print(f"[mr-proj {time.strftime('%H:%M:%S')}] {msg}", flush=True)
...
def mr_wald(beta_e: float, se_e: float, beta_o: float, se_o: float) -> tuple[float, float, float]:
...
def mr_ivw(rows: list[HarmRow]) -> dict:
    if not rows:
    ...
    for r in rows:
        ...
        ws.append(w); bxs.append(r.beta_exposure); bys.append(r.beta_outcome)
    if not ws:
        return {"beta": math.nan, "se": math.nan, "p": math.nan, "n": 0, "Q": math.nan}
...
def mr_weighted_median(rows: list[HarmRow]) -> float:
    if len(rows) < 3:
        return math.nan
    ratios, weights = [], []
    for r in rows:
...
def run_mr(rows: list[HarmRow]) -> dict:
    if not rows:
        return {"n_instruments": 0, "method": "no_signal",
                "beta": math.nan, "se": math.nan, "p": math.nan,
                "Q": math.nan, "weighted_median_beta": math.nan,
                "neg_log10p": None}
    if len(rows) == 1:
...
def benjamini_hochberg(pvals: list[float]) -> list[float]:
    """BH-FDR. Returns adjusted q-values aligned to input order."""
    n = len(pvals)
    if n == 0:
        return []
    ...
    for rank, (p, orig_i) in enumerate(reversed(indexed_sorted), start=1):
        # rank counts from largest p downward
        k = n - rank + 1
...
def main() -> int:
    ...
    instr = instr.filter(pl.col("af_match") == "matched")  # use harmonized only
    _log(f"loaded {instr.height} matched instruments / {instr['aptamer'].n_unique()} aptamers")

    instr_b37 = liftover_b38_to_b37(instr)

    af_df = pl.read_parquet(OUTCOMES["AF"]["path"]).select([
        "chr", "pos", "rsid", "effect_allele", "other_allele",
        "beta", "se", "pval", "neg_log10p", "eaf",
    ])
    _log(f"AF parquet loaded ({af_df.height:,})")

    # ----- 3B AF MR (per aptamer) -----
    mr_rows = []
    af_harm_qc = []
    by_apt = instr.group_by("aptamer").agg(pl.first("gene").alias("gene"))
    for r in by_apt.iter_rows(named=True):
        apt = r["aptamer"]
        gene = r["gene"]
        sub = instr.filter(pl.col("aptamer") == apt)
        kept, counters = harmonize(sub, af_df, build="GRCh38")
        counters["aptamer"] = apt
        counters["gene"] = gene
        af_harm_qc.append(counters)
        m = run_mr(kept)
        mr_rows.append({
            "aptamer": apt, "gene": gene,
            **m, "smoke_only": False, "calibration_only": False,
            "scope": "CVD454_staged_discovery",
        })
    pl.DataFrame(af_harm_qc).write_csv(OUT_AF_HARM, separator="\t")
    _log(f"wrote {OUT_AF_HARM}")

    # BH-FDR within CVD454 universe
    pvals = [r["p"] for r in mr_rows]
    fdrs = benjamini_hochberg(pvals)
    for r, q in zip(mr_rows, fdrs):
        r["fdr_bh_within_cvd454"] = q
    pl.DataFrame(mr_rows).write_csv(OUT_AF_MR, separator="\t")
    _log(f"wrote {OUT_AF_MR}")

    # ----- Candidate shortlist -----
    candidates = []
    for r in mr_rows:
        p = r["p"]; q = r["fdr_bh_within_cvd454"]
        if p is None or not math.isfinite(p):
            label = "AF_no_signal"
        elif q is not None and math.isfinite(q) and q < 0.05:
            label = "AF_FDR_candidate"
        elif p < 0.05:
            label = "AF_nominal_candidate"
        else:
            label = "AF_no_signal"
        sanity = r["gene"] in SANITY_TARGETS
        carry = (label in ("AF_FDR_candidate", "AF_nominal_candidate") or sanity)
        d_T = 0
        ...
        candidates.append({
            ...
            "p_T_AF": r["p"], "fdr_bh_within_cvd454": r["fdr_bh_within_cvd454"],
            ...
            "scope": "CVD454_staged_discovery_NOT_proteome_wide",
        })
    pl.DataFrame(candidates).write_csv(OUT_CANDIDATES, separator="\t")
    _log(f"wrote {OUT_CANDIDATES}")

    n_fdr = sum(1 for c in candidates if c["candidate_label"] == "AF_FDR_candidate")
    n_nom = sum(1 for c in candidates if c["candidate_label"] == "AF_nominal_candidate")
    n_no = sum(1 for c in candidates if c["candidate_label"] == "AF_no_signal")
    n_carry = sum(1 for c in candidates if c["carry_to_projection"])
    _log(f"AF MR: FDR_candidate={n_fdr}  nominal={n_nom}  no_signal={n_no}  "
         f"carry_to_projection={n_carry}")
    ...
    for outcome_label, meta in OUTCOMES.items():
        ...
        if not meta["path"].exists():
            _log(f"skip {outcome_label}: parquet missing")
            continue
        outcome_df = pl.read_parquet(meta["path"])
        _log(f"{outcome_label} (build {meta['build']}, role {meta['role']}, "
             f"{outcome_df.height:,} rows)")

        instr_for = instr_b37 if meta["build"] == "GRCh37" else instr
        # Restrict to carry-aptamers only
        sub = instr_for.filter(pl.col("aptamer").is_in(list(carry_aptamers.keys())))

        for apt in carry_aptamers:
            cand = carry_aptamers[apt]
            apt_sub = sub.filter(pl.col("aptamer") == apt)
            if apt_sub.height == 0:
                continue
            kept, counters = harmonize(apt_sub, outcome_df, build=meta["build"])
            m = run_mr(kept)
            d_T = cand["d_T"]
            ...
            projection_rows.append({
                ...
                "d_T": d_T,
                **{f"{k}_outcome": m[k] for k in ("beta", "se", "p")},
    ...
    _log(f"wrote {OUT_DIRECTION}")

    # ----- Final candidate summary md -----
    n_with_label = sum(1 for r in projection_rows if r["provisional_labels"])
    md = [
        "# CVD454 staged discovery — candidate summary (D035, NOT proteome-wide)",
        "",
        f"**Scope:** CVD-prioritized deCODE pQTL target screen (454 aptamers; not proteome-wide).",
        f"**FDR:** Benjamini-Hochberg WITHIN the CVD454 aptamer universe (n={len(mr_rows)}).",
        "",
        "## AF anchor MR (Phase 3B)",
        "",
        f"- AF_FDR_candidate: **{n_fdr}**",
        f"- AF_nominal_candidate: **{n_nom}**",
        f"- AF_no_signal: **{n_no}**",
        ...
        "",
        "**STILL LOCKED** — protocol explicit approval required for Phase 3D coloc.",
    ]
